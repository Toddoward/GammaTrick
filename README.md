...
**검은 테두리도 `gAMA`나 ICC 청크로 만들면 더 효율적이지 않나요?**

청크로는 만들 수 없습니다.

- 청크는 이미 있는 픽셀의 **색을 다르게 해석**하게 할 뿐, 없는 픽셀을 **만들어 내지는 못합니다**. 테두리는 이미지 바깥에 공간을 더하는 것이라 실제 픽셀이 있어야 합니다.
- 테두리가 꼭 보여야 하는 곳은 **썸네일**입니다. 그런데 썸네일은 청크를 버린 상태라, 청크에 담은 정보는 썸네일에 전혀 나타나지 않습니다.
- 테두리 글씨가 "썸네일에서는 보이고 원본에서는 사라지는" 것도 같은 원리입니다. 글씨는 실제 픽셀(밝은 회색 235)로 들어가 있고, 청크는 원본 보기에서 그 픽셀을 검정으로 바꿔 보이게 할 뿐입니다.

비용도 거의 없습니다. 테두리는 단색이라 거의 압축되고, 디더링도 하지 않습니다. 560×560 이미지 기준으로 테두리 15px는 파일을 약 2KB(1.3%) 늘렸고, 처리 시간 차이는 측정되지 않았습니다.
...
| `res/gamma.js` | 화면 동작: 파일 입력, 옵션, 테두리 문구, 테마 전환, 결과 표시 |
| `res/gamma.css` | 스타일 |
