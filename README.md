...
- **알파 채널을 함께 넣습니다.** 픽셀은 전부 불투명(255)이라 보이는 건 똑같지만, 알파가 없는 PNG는 안드로이드 앱이 메모리를 아끼려고 채널당 5~6비트(RGB565)로 줄여서 읽을 수 있습니다. 이 방식은 정보를 흰색 바로 아래 몇 단계에 담기 때문에 그렇게 줄이면 통째로 날아갑니다. 알파가 있으면 그 강등이 일어나지 않습니다. 파일은 약 11% 커집니다.
- **색 변환표를 못 읽는 뷰어에서도 위장 이미지는 제 색으로 보입니다.** ICC 프로필에 "이건 그냥 sRGB"라는 예비 정보를 함께 적어 둡니다. 3D 변환표(A2B0)를 건너뛰는 뷰어는 숨긴 그림을 못 풀 뿐, 위장 이미지나 흰 화면은 의도한 색 그대로 그립니다.
...
- **고화질(ICC)은 Windows 사진 앱에서 깨질 수 있습니다.** 보통 5단계부터, 이미지에 따라서는 4단계에서도 얼룩덜룩한 거짓 색으로 보입니다. 프로필 크기나 단계 수만으로는 설명되지 않아서 원인을 아직 특정하지 못했습니다. 브라우저(Chromium 계열)와 게시판에서는 정상입니다. 기본값을 4단계로 둔 이유입니다.
- **게시판 모바일 앱으로 올리면 결과가 다를 수 있습니다.** 앱이 올리기 전에 이미지를 다시 가공하는 경우가 있어, PC 웹으로 올린 것과 다르게 보일 수 있습니다. 확실하게 하려면 PC 웹에서 올리세요.
